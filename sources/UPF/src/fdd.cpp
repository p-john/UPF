  ...
  for (auto edge : node->edges_){
    if(edge->covers(range)){
      ...
      if(field_key != tcpflagfield){
        return have_same_decision(edge->target_node_,rule);
        //return true;
      }
...
//    std::cout << *t_d.root() << std::endl;
    ...
    for(int i = ruleset.size() - 2; i >= 0; --i){
      ...
      //std::cout << "Checking Rule " << i << std::endl << ruleset.get_rule(i) <<  std::endl;
...
//        std::cout << *t_d.root() << std::endl;
        ...
        ;
       //std::cout << "Removing Downward Redundant Rule  " << i << " : " << ruleset.get_rule(i) << std::endl;
      }
