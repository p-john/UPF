...
uint128_t dec_tree::find_best_cut(const UPF_Ruleset& ruleset,
...
void dec_tree::collect_ranges(const UPF_Ruleset& ruleset,
                              const DimensionType dim,
...
void dec_tree::collect_endpoints(const UPF_Ruleset& ruleset,
                                 const DimensionType dim,
                                 std::vector<uint128_t>& endpoints){
...
void dec_tree::collect_endpoints_in_box(const UPF_Ruleset& ruleset,
                                        const DimensionType dim,
                                        const Bounding_Box& box,
                                        std::vector<uint128_t>& endpoints){
  for(unsigned int i = 0; i < ruleset.size(); ++i){
...
uint128_t dec_tree::equal_segments(const std::vector<uint128_t>& endpoints,
                                   const DimensionType dim ,
                                   const Bounding_Box& box){
...
uint128_t dec_tree::equal_rules(const UPF_Ruleset& ruleset,
                                const DimensionType dim,
                                const Bounding_Box& box,
...
uint128_t dec_tree::weighted_segments(const UPF_Ruleset& ruleset,
                                      const DimensionType dim,
                                      const Bounding_Box& box,
                                      const std::vector<uint128_t>& endpoints){
...
void dec_tree::limit_rule_on_box(const UPF_Rule& rule,
                                 Bounding_Box& box){
...
void dec_tree::get_most_distinct_rules(Node* current_node,
...
void dec_tree::get_most_distinct_endpoints(Node* current_node,
                                        std::vector<Distinct>& distincts){
...
bool has_endpoints(dec_tree::Node* current_node, const DimensionType dim){
...
dec_tree::DimCut dec_tree::determine_best_dimcut(Node* current_node,
                                                 const CuttingType cutting_type){
...
void dec_tree::duplicate_intersected_rules(
...
void dec_tree::build_child_node(Node* current_node,
  ...
  if(child_ruleset.size() > 0){
    dec_tree::Node* child = current_node->create_child(child_ruleset,
                                                       child_box,
                                                       number);
...
void two_node_hicut(dec_tree::Node* current_node,
...
void three_node_hypersplit(dec_tree::Node* current_node,
                           std::queue<dec_tree::Node*>& nodes_to_process,
                           unsigned int& number_nodes,
                           const CuttingType cutting_type){
...
void dec_tree::Node::split_protocols(std::queue<Node*>& nodes_to_process,
                                     unsigned int& number_nodes){
...
void dec_tree::Node::collect_rulesets(
...
dec_tree::Node::Node(UPF_Ruleset& ruleset)
    :   name_("root"),
        num_childs_(0),
...
dec_tree::Node::Node(UPF_Ruleset& ruleset, const std::string& tree_name)
    :   name_("root"),
        num_childs_(0),
        ruleset_(std::move(ruleset)),
        node_number_(0),
...
dec_tree::Node::Node(UPF_Ruleset& ruleset, const std::string& name,
...
dec_tree::Node* dec_tree::Node::create_child(UPF_Ruleset& ruleset,
                                             const Bounding_Box& box,
                                             unsigned int& number){
...
UPF_Ruleset& dec_tree::Node::get_ruleset(){
  return ruleset_;
}

uint64_t dec_tree::Node::depth() const{
...
const std::string& dec_tree::Node::get_name() const{return name_;}

unsigned int dec_tree::Node::number_of_rules() const{
  uint64_t rules = 0;
  for(unsigned int i = 0; i < ruleset_.size(); ++i)
...
void dec_tree::Node::cut(DecisionTree& tree,
                         const CuttingType cutting_type,
                         const unsigned int threshold){

//    std::cout << "Number of Rules: " << ruleset_.size();
//    for(unsigned int i = 0; i < ruleset_.size(); ++i)
//      if (ruleset_.get_rule(i).has_unknown_match())
//        std::cout << ruleset_.get_rule(i);
//    std::cout << "MAX OVERLAPPING RULES: " << max_overlapping(ruleset_) << std::endl;
...
bool dec_tree::Node::is_cuttable(){
...
dec_tree::DecisionTree::DecisionTree(UPF_Ruleset& ruleset,
                                     const std::string& tree_name,
...
void dec_tree::DecisionTree::create_tree(const CuttingType cut_type,
...
dec_tree::Bounding_Box::Bounding_Box()
...
dec_tree::Bounding_Box::Bounding_Box(const Bounding_Box& rhs)
...
void dec_tree::Bounding_Box::print() const{
...
bool dec_tree::Bounding_Box::covers(const Bounding_Box& rhs) const{
...
void dec_tree::Bounding_Box::set_unknown(const bool unknown){
  unknown_match_ = unknown;
}

void dec_tree::Bounding_Box::set_range(const DimensionType dim,
                                       const Range& range){
...
inline const Range& dec_tree::Bounding_Box::get_range(const DimensionType dim) const{
...
inline const uint128_t& dec_tree::Bounding_Box::get_lower_bound(const DimensionType dim) const{
...
inline const uint128_t& dec_tree::Bounding_Box::get_upper_bound(const DimensionType dim) const{
